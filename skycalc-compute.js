...
function computeObservability(state, startJD, endJD, sunAltDeg) {
  ...
  let nlun = la0.nlun;
  // Back up one to ensure we don't miss the first event
  nlun = nlun - 1;

  const results = [];

  for (let iter = 0; iter < 60; iter++) {
    for (const nph of [0, 2]) { // 0=new, 2=full
      const jdPhase = flmoon(nlun, nph);
      if (jdPhase < startJD - 2 || jdPhase > endJD + 2) continue;

      const { zone } = getDSTInfo(jdPhase, site);
      const locCal   = jdToLocalCal(jdPhase, zone);
      const y = locCal.y, mo = locCal.mo, d = locCal.d;
      const jdMidnightLocal = dateToJD(y, mo, d, 24, 0, 0);
      const jdmid = jdMidnightLocal + zone / 24;

      if (jdmid < startJD || jdmid > endJD) continue;

      const curEpoch = 2000.0 + (jdmid - J2000) / 365.25;
      const prec = precrot(ra, dec, epoch, curEpoch);

      // Evening/morning twilight — hour-angle-based guesses at midnight
      // (skycalc.c method), not fixed ±6 h clock offsets.
      const stmid  = lst(jdmid, site.longit);
      const sunMid = lpsun(jdmid);
      const haTwi  = haAlt(sunMid.dec, site.lat, sunAltDeg);
      let jdEveT = null, jdMornT = null;
      if (haTwi <= 900 && haTwi >= -900) {
        const e = jdSunAlt(sunAltDeg, jdmid + adjTime(sunMid.ra + haTwi - stmid) / 24, site.lat, site.longit);
        const m = jdSunAlt(sunAltDeg, jdmid + adjTime(sunMid.ra - haTwi - stmid) / 24, site.lat, site.longit);
        jdEveT  = e > -999 ? e : null;
        jdMornT = m > -999 ? m : null;
      }

      function objAt(jdt) {
        const sid = lst(jdt, site.longit);
        const ha  = adjTime(sid - prec.ra);
        const { alt } = altit(prec.dec, ha, site.lat);
        const secz = secantZ(alt);
        return { ha, secz, alt };
      }

      // Center of the dark window = midpoint of the two twilights (skycalc.c's
      // "center of night" concept), not clock midnight.
      const jdCtr = (jdEveT && jdMornT) ? (jdEveT + jdMornT) / 2 : jdmid;
      const eve  = jdEveT  ? objAt(jdEveT)  : null;
      const ctr  = objAt(jdCtr);
      const morn = jdMornT ? objAt(jdMornT) : null;

      // Count hours accessible
      let hrs3 = 0, hrs2 = 0, hrs1p5 = 0;
      if (jdEveT && jdMornT) {
        const step = 0.1 / 24;
        for (let jdt = jdEveT; jdt <= jdMornT; jdt += step) {
          const { secz, alt } = objAt(jdt);
          if (alt > 0 && secz < 100) {
            hrs3   += (secz < 3.0)   ? step * 24 : 0;
            hrs2   += (secz < 2.0)   ? step * 24 : 0;
            hrs1p5 += (secz < 1.5)   ? step * 24 : 0;
          }
        }
      }
      ...
      results.push({
        jdPhase,
        phase: nph === 0 ? 'N' : 'F',
        dateStr: fmtDate(locCal),
        eve, ctr, morn,
        hrs3:   hrs3.toFixed(1),
        hrs2:   hrs2.toFixed(1),
        hrs1p5: hrs1p5.toFixed(1),
      });
    }
    nlun++;
    if (flmoon(nlun, 0) > endJD + 60) break;
  }

  results.sort((a, b) => a.jdPhase - b.jdPhase);
  return results;
}
