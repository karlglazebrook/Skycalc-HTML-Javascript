/* Karl Glazebrook's customized version */
/* Added APO site (removed Shattuck) - kgb apr/12/2000 */
/* Added Baltimore site (removed Black Moshannon) - kgb Nov/2/2000 */
/* Fixed Australian Daylight Saving time boundaries - karl Mar/6/2010 */
/* Added ESO VLT site (also fixed minor return value bug line 5687) and
   updated Chilean daylight saving time definitions (approximately,
   it is complicated!) - kgb sep/11/2018 */
...
/* overkill to put in these preprocessor flags, perhaps. */
		...
		case 'h':    /* signals short argument ... */
			/* Note Karl note short is promoted to int */
			shval = va_arg(ap, int);
			outform[++i] = 'd';
			outform[++i] = '\0';
			++p;  /* skip the 'd' in '%hd' */
...

{
	...
	printf("   v .. VLT European Southern Obs, Paranal\n");
	...
	printf("   s .. SDSS - Apache Point Observatory New Mexico\n");
	printf("Your answer --> ");
	scanf("%s",obs_code);
	if(obs_code[0] == 'x') {
	...
	if(obs_code[0] == 'k') {
		...
		*elev = 700.;  /* approximate -- to match KPNO tables */
		...
		strcpy(site_name, "Apache Point Observatory");
		strcpy(zone_name,"Mountain");
		*zabr = 'M';
		*use_dst = 0;
		*longit = 7.054666666666;
		*lat = 32.78055555555;
		*stdz = 7.;
		*elevsea = 2800.;
		*elev = 2800.;  /* Assume flat horizon */
	}
		...
		*elev = 2347.; /* for ocean horizon, not Andes! */
		...
		strcpy(site_name, "VLT, ESO Cerro Paranal");
		strcpy(zone_name, "Chilean");
		*zabr = 'C';
		*use_dst = -1;
		*longit = 4.6944;
...

{
	...
	if((use_dst == 1) || (use_dst == 0)) {
	   ...
	   /* now from 2019 onward, this begins on the first Saturday of April 
	   and ends on the first Saturday of September */
		trial.y = yr;
		trial.mo = 4;
		trial.d = 7;  /* earliest possible first Sat */
		trial.h = 2;
		trial.mn = 0;
		trial.s = 0;
	   ...
	   /* off daylight 1st Sun in April, onto daylight first Sun in October */
		trial.y = yr;
		trial.mo = 4;
